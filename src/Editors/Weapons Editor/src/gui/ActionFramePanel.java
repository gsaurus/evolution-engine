...
public class ActionFramePanel extends APanelWithScroll implements MouseMotionListener, MouseListener, ICoordinatesTaker{
    ...
    protected List<UBPoint> actionPoints;
    ...
    public void setFrame(Frame f){
     ...
     /** set frame */
    public void setActionPoints(List<UBPoint> points){
    ...
    public List<UBPoint> getPoints(){ return actionPoints; }
    ...
    protected int catched(int x, int y){
        ...
        for(int i=0; i<actionPoints.size(); i++){
            UBPoint p = actionPoints.get(i);
            int cx = (int)(p.getX()*scaleFactor+(0.5*scaleFactor));
            int cy = (int)(p.getY()*scaleFactor+(0.5*scaleFactor));
	...
	public void mouseDragged(MouseEvent e) {
        ...
        if (selected == -1 || actionPoints == null ) return;
        int oldX = actionPoints.get(selected).getX();
        int oldY = actionPoints.get(selected).getY();
        actionPoints.get(selected).set((int)(mouseX/scaleFactor),(int)(mouseY/scaleFactor));
        repaintCursor(oldX,oldY);
        final int newX = actionPoints.get(selected).getX();
        final int newY = actionPoints.get(selected).getY();
        repaintCursor(newX,newY);
        coordinatesTaker.takeCoordinates(newX,newY, selected);
	}
	...
	public void mousePressed(MouseEvent e) {
        ...
        if (tryCatch != -1){
            selected = tryCatch;
            coordinatesTaker.takeCoordinates((int)(mouseX/scaleFactor),(int)(mouseY/scaleFactor), selected);
            repaint();
        }
    }
    ...
    protected void drawCross(Graphics g, int cx, int cy){
